...
export function getTileBounds(tileIndex: TileIndex): {
  lngMin: number;
  latMin: number;
  lngMax: number;
  latMax: number;
} {
  const n = 2 ** tileIndex.z;
...
export function tileIdToTileIndex(id: string): TileIndex {
  ...
  return {
    z: Number.parseInt(splitted[0], 10),
    x: Number.parseInt(splitted[1], 10),
    y: Number.parseInt(splitted[2], 10),
  };
}
